...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            // ===== 1. ADMIN LOGIN CHECK (NEW) =====
            if ($user->role === 'admin') {
                // Redirect admins directly to their dashboard/list
                // Ensure this route exists in your web.php
                return redirect()->route('admin.developers.index'); 
            }

            // ===== 2. STORE CUSTOMER RELATED SESSION FOR NON-DEVELOPERS =====
    ...
    public function subscriberLogin($uuid)
    {
        ...
        $subscriber = User::where('uuid', $uuid)->first();
        if (!$subscriber) {
